...
export async function sendMagicLinkEmail({ email, url }: MagicLinkEmailInput) {
  ...
  const { error } = await resend.emails.send({
    ...
    to: email,
    subject: "Mutar login link",
    text: `Open this link to log in to Mutar:\n\n${url}\n\nIf you did not request this email, you can ignore it.`,
    html: `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; color: #191714; line-height: 1.6;">
        <p style="margin: 0 0 16px;">${safeEmail} で Mutar にログインするためのリンクです。</p>
        <p style="margin: 0 0 24px;">このリンクは短時間で期限切れになります。</p>
        <a href="${safeUrl}" style="display: inline-block; border-radius: 999px; background: #191714; color: #fff; padding: 12px 20px; text-decoration: none;">Mutar にログイン</a>
  ...
  if (error) {
    console.error("[mutar] failed to send magic link email", error)
