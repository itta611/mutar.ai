            ...
            <DropdownMenu>
              <DropdownMenuTrigger render={
                <SidebarMenuButton className="h-15 gap-4">
                  ...
                  <div className="grow h-10 flex flex-col justify-center gap-0.5">
                    <div className="font-semibold">{user ? user.name : <Skeleton className="h-4 w-12 mb-1.5" />}</div>
                    <div className="text-xs text-muted-foreground">{user ? user.email : <Skeleton className="h-3 w-24" />}</div>
                  </div>
