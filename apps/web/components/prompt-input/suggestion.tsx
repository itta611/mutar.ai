...
const SuggestionButton = (props: ComponentProps<"button">) => {
  return (
    <button
      className="rounded-full text-sm h-8 px-4 text-muted-foreground transition cursor-pointer border bg-background hover:bg-muted"
      {...props}
    />
  )
}
...
「受領」「OCR読み取り」「承認」「支払い」「保存」の5ステップを左から右に配置。
